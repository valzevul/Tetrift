...
class GameScene: SKScene {
    ...
    var tick: (() -> ())?
    var tickLengthMilles = TickLengthLevelOne
    var lastTick: NSDate?
    
    var textureCash = Dictionary<String, SKTexture>()
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if timePassed > tickLengthMilles {
    ...
    func addPreviewShapeToScene(shape:Shape, completion:() -> ()) {
        for (idx, block) in enumerate(shape.blocks) {
            var texture = textureCash[block.spriteName]
            if texture == nil {
                texture = SKTexture(imageNamed: block.spriteName)
                textureCash[block.spriteName] = texture
            }
